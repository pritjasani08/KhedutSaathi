    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        ...
        <motion.div
          ...
          className="relative w-full max-w-5xl bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] border border-slate-800"
        >
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 z-20 p-2.5 bg-slate-800/90 shadow-lg hover:bg-slate-700 rounded-full text-slate-300 transition-colors border border-slate-700"
          >
            <X className="w-5 h-5" />
          </button>

          <div className="md:w-1/2 bg-slate-950 relative min-h-[300px] flex flex-col overflow-hidden p-6">
            <div className="flex-1 relative rounded-2xl overflow-hidden border border-slate-800 shadow-inner bg-slate-900 mb-4">
              <img 
                src={images[activeImageIndex]} 
                alt={product.name}
                className="w-full h-full object-cover opacity-90"
              />
              <div className="absolute top-4 left-4 bg-slate-900/90 border border-slate-700 backdrop-blur px-3 py-1.5 rounded-xl text-sm font-bold text-green-400 shadow-sm z-10">
                {product.category}
              </div>
                ...
                {images.map((img, idx) => (
                  <button 
                    ...
                    className={cn(
                      ...
                      activeImageIndex === idx ? "border-green-500 shadow-lg shadow-green-500/20" : "border-slate-800 opacity-50 hover:opacity-100"
                    )}
                  >
          ...
          <div className="md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto custom-scrollbar">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-lg">{product.seller?.full_name || product.brand || 'Verified Seller'}</span>
              <div className="flex items-center gap-1.5 bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-lg text-amber-400 text-xs font-bold">
                <Star className="w-4 h-4 fill-amber-400" />
                {product.rating || '4.5'} <span className="text-slate-500 font-medium">({product.reviewsCount || '12'} reviews)</span>
              </div>
            </div>

            <h2 className="font-display text-3xl md:text-4xl font-bold text-white mb-6 tracking-tight leading-tight">{product.name}</h2>
            
            <div className="flex flex-wrap items-end gap-4 mb-8 border-b border-slate-800 pb-8">
              <div className="flex items-center font-display text-4xl font-bold text-white">
                <IndianRupee className="w-8 h-8 mr-1 text-green-400" />
            ...
            <div className="prose prose-sm prose-invert mb-10 max-w-none text-slate-300">
              <p className="text-base leading-relaxed">{product.description}</p>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-10 mt-auto">
              <div className="flex items-center gap-4 p-5 bg-slate-800/50 rounded-2xl border border-slate-800">
                ...
                <div>
                  ...
                  <span className="text-base font-bold text-slate-200">{product.seller?.full_name || 'Verified Seller'}</span>
                </div>
              </div>
              <div className="flex items-center gap-4 p-5 bg-slate-800/50 rounded-2xl border border-slate-800">
                <div className="p-2.5 bg-slate-800 rounded-xl border border-slate-700 shadow-inner">
                ...
                <div>
                  ...
                  <span className="text-base font-bold text-slate-200">Direct Message</span>
                </div>
              </div>
            </div>
              ...
              <button 
                ...
                className={cn(
                  ...
                  isFavourited 
                    ...
                    : "border-slate-700 bg-slate-800 text-slate-400 hover:text-red-400 hover:border-red-500/50 hover:bg-red-500/10"
                )}
              >
