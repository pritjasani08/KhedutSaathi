...
export default function ProductCard({ product, onClick, onAddToCart }) {
  ...
  return (
    <div
      className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-2xl hover:shadow-black/10 dark:hover:shadow-black/40 hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden group h-full relative"
    >
      ...
      <div className="absolute inset-x-0 top-[110px] -translate-y-1/2 z-20 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto">
        <button 
          ...
          className="bg-white/90 dark:bg-slate-900/90 backdrop-blur text-slate-900 dark:text-white font-bold px-6 py-2.5 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 hover:bg-green-500 hover:text-white dark:hover:bg-green-600 hover:border-green-500 dark:hover:border-green-500 transition-colors flex items-center gap-2"
        >
      ...
      {/* Image Area */}
      <div className="relative h-56 bg-slate-100 dark:bg-slate-900 overflow-hidden cursor-pointer flex-shrink-0" onClick={() => onClick(product)}>
        <img 
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
          ...
          <span className="bg-white/80 dark:bg-slate-900/80 backdrop-blur text-green-600 dark:text-green-400 border border-slate-200 dark:border-slate-700 text-xs font-bold px-2.5 py-1 rounded-lg shadow-sm">
            {product.category}
          </span>
        </div>
        ...
        {/* Stock Overlay */}
        {product.stock === 0 && (
          <div className="absolute inset-0 bg-slate-900/20 dark:bg-slate-950/60 backdrop-blur-[2px] flex items-center justify-center z-10">
            <span className="bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-bold px-4 py-1.5 rounded-full text-sm shadow-lg border border-slate-200 dark:border-slate-700">
              Out of Stock
            </span>
          </div>
        )}
      ...
      <div className="p-5 flex flex-col flex-1 bg-white dark:bg-slate-800">
        <div className="flex justify-between items-start mb-3">
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{product.seller?.full_name || product.brand || 'Verified Seller'}</p>
          <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded text-amber-500 dark:text-amber-400 text-xs font-bold">
            <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
            {product.rating} <span className="text-slate-400 dark:text-slate-500">({product.reviewsCount})</span>
          </div>
        </div>

        <h3 
          className="font-display font-bold text-slate-900 dark:text-white text-lg leading-tight line-clamp-2 mb-3 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors cursor-pointer"
        ...
        <div className="flex items-end justify-between mt-auto pt-4 border-t border-slate-200 dark:border-slate-700">
          <div>
            ...
            <p className="text-xl font-bold text-slate-900 dark:text-white flex items-center">
              <IndianRupee className="w-5 h-5 mr-0.5 text-green-600 dark:text-green-400" />
