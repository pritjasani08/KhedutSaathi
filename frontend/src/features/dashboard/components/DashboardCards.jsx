...
import { WeatherSkeleton, MarketSkeleton } from '../skeletons/Skeletons';
...
export const WelcomeHeader = ({ user }) => (
  <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="mb-8">
    <motion.div variants={fadeUp} className="flex items-center gap-3 mb-2">
      ...
      <h1 className="font-display text-3xl font-bold text-heading">Welcome back, {user.first_name}!</h1>
    </motion.div>
...
export const FarmSummaryCard = ({ profile }) => (
...
export const ProfileCompletionCard = ({ completionPercentage }) => (
...
export const WeatherCard = ({ profile }) => {
  const { data: weather, isLoading, error } = useWeather(profile?.state || 'Gujarat');
