...
export default function SchemeEligibilityEngine() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/schemes/eligible`, {
        method: 'POST',
      ...
      if (user && fullProfile) {
        try {
          const token = localStorage.getItem('token');
          await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/profile`, {
            method: 'PUT',
            headers: { 
